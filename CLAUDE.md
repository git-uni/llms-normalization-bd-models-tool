...
1. **Arreglar `build_tree_summary` para repos grandes.** Esto destapado en el punto 25 es el bloqueo real de la pasada implícita: cualquier prompt que pida "mira el árbol y busca evidencia más allá del grep declarativo" es ineficaz si el árbol no incluye los dirs candidatos. Dos opciones concretas:
   - **BFS en lugar de DFS** con el mismo cap de 600: garantiza que todos los top-level dirs aparezcan antes de profundizar. Cambio puntual de algoritmo en `filesystem.py::build_tree_summary`.
   - **Subir `max_entries`** (a 1500-2000) — más simple, sube TPM de partida pero todavía cabe en el 250K cap de Gemini 3.1 Flash Lite.
   Recomendado: BFS primero, ver si con 600 entradas BFS se ven los dirs interesantes; subir el cap solo si BFS no basta.
2. **El batching nudge del prompt v5 no se sostiene entre runs.** Run C (con batching, 2 iter) y Run D (sin batching, 23 iter) tienen el mismo prompt. Posibles vías: (a) reforzar el batching como **regla dura** y no como sugerencia al final del prompt; (b) aceptar que es no determinista y el RPM cap es el techo real. Antes de tocar, validar contra más runs si la varianza se mantiene.
3. **Fix puntual del retry de Google para 5xx.** `_call_with_retry` solo trata 429. Gemma ya ha devuelto 500/503 transitorios al menos 3-4 veces. Añadir 500/502/503/504 al backoff es ~5 líneas en `providers/google.py`. Independiente del resto.
