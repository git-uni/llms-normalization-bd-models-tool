...
1. **Cablear `CerebrasProvider`** copiando `groq.py` y probar con `qwen-3-235b-a22b-instruct-2507`. Es la única vía free hoy que combina (a) modelo más capable que Qwen 32B, (b) cuota razonable (1M TPD), (c) integración trivial. Antes verificar en `inference-docs.cerebras.ai/llms.txt` que ese modelo soporta tools (los docs explícitos solo cubren `gpt-oss-120b` y `zai-glm-4.7`). Probable necesidad de `time.sleep(~12s)` defensivo para no chocar con los 5 RPM.
2. **Re-validar Gemini 2.5 Flash mañana** con la cuota fresca. En el run abortado de hoy estaba cubriendo los 4 schemas de `utils/models/` antes de petar — punto de comparación directo Google vs Groq vs Cerebras.
3. **Considerar bajar `MAX_ITERS`** o añadir un cap blando — si el agente necesita >15 iter quizá es señal de prompt aún demasiado abierto.
