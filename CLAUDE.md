...
1. **Atacar el techo de cobertura en proyectos reales sin depender solo del prompt.** El punto 24 confirma que el refinamiento del prompt solo cubre la mitad del gap — el agente sigue dejando hermanos legítimos fuera. Tres líneas concretas:
   - **Manifiesto enriquecido en el primer turno**: el árbol filtrado actual da paths + tamaños. Si en su lugar (o además) presentamos paths + las primeras ~20-30 líneas de cada archivo de código, el modelo "ve" desde el principio que `coupon.js` define un schema sin necesidad de leerlo a propósito. Convierte el patrón "explorar y decidir" en "filtrar de lo que ya viste". Cambio acotado en `discovery/filesystem.py::build_tree_summary` + alguna línea en `agent.py`. Trade-off: TPM más alto de partida; con el cap de 250K aún sobra para repos medianos. Idea original del usuario, en standby hasta ahora.
   ...
   - **Caching de `read_file` en `DiscoveryState`** (idea del Plan agent del 24): si el modelo lee el mismo archivo dos veces, ahorra iter. Ya se ha visto en runs anteriores.
2. **Fix puntual del retry de Google para 5xx.** `_call_with_retry` solo trata 429. Gemma ya ha devuelto 500/503 transitorios al menos 3-4 veces (punto 24). Añadir 500/502/503/504 al backoff es ~5 líneas en `providers/google.py`. Independiente del refinamiento del prompt.
3. **Si el RPM cap del free tier se confirma bloqueante** en proyectos más grandes que Habitica, las opciones son:
