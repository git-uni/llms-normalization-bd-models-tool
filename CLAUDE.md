...
13. **Gotcha de Llama en Groq:** los modelos `llama-3.1-8b-instant` y `llama-3.3-70b-versatile` emiten function calls con sintaxis markup (`<function=name>args</function>`) en lugar del slot estructurado `tool_calls` que espera la API de Groq (formato OpenAI), y la API los rechaza con `tool_use_failed`. Por eso el default del agente en Groq NO es Llama. `openai/gpt-oss-20b` usa el slot correcto pero a veces emite JSON truncado en los argumentos. **`qwen/qwen3-32b` funciona consistentemente** y es el default. Los Llama siguen valiendo para el pipeline (texto-a-texto, sin tools).
